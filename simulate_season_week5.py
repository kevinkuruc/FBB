...
def simulate_regular_season(schedule, strengths):
    """
    Simulate the remaining 15-week regular season starting from STARTING_RECORDS.

    Returns dict: team -> {"wins": int, "losses": int, "ties": int}
    where wins/losses/ties are cumulative category results across all 20 weeks
    (5 actual + 15 simulated).
    """
    records = {team: dict(STARTING_RECORDS[team]) for team in ALL_TEAMS}
