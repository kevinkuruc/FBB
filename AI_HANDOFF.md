...
19. Trade analysis tool: `trade_tool.html` (generated by `build_trade_tool.py` from `draft_tool.html` + a Depth Charts ROS CSV + a rosters CSV). Hitter projections are recomputed inside the build script from `Depth_Charts_May_ROS.csv` using the same `TARGET_PA=625` supplement as `create_league_stats.py`; a `value` field (zTotal) is also embedded so the lineup optimizer can break position ties by fantasy value. All 16 rosters are pre-loaded from `rosters_May.csv` — every owned player and every FA stays in the pool as a potential trade target. The build script also replaces the position optimizer with one that pools all of a team's hitters (active + bench), sorts by most-constrained-first with `value` DESC tie-break, prefers RARER slots (so Arraez 1B/2B grabs 2B when 2B has no other claimants), then runs two passes (non-UTIL then UTIL); any player who can't fit an eligible slot goes to `team.benchHitters` and an empty slot contributes HITTER_REP instead. This eliminated the pre-fix bug where the loader cut Skrey's only catcher (Moreno) by RkOv-ordered intake and the optimizer's last-resort fallback parked Moniak at C. `calculateMarginalValue` models an acquisition: for each candidate, try every drop (active OR bench) from Skrey's roster, re-run the optimizer, and keep the swap with the highest total wins. The "+MV" column is therefore (best post-trade wins − current wins). The position filter (C, 1B, ...) lets you sort all eligible targets by upside; each row carries an owner badge. To refresh: drop new files into `/home/user/FBB/`, point `MAY_ROS_FILE` / `ROSTERS_CSV` in `build_trade_tool.py` at them, and re-run `python3 build_trade_tool.py`.

20. Trade Deadline Analyzer: `deadline_tool.html`, generated by `build_deadline_tool.py` from `deadline_tool_template.html` (a from-scratch app — it does NOT patch draft_tool.html). Hitters come from `TheBat_RoS_ASB.csv` (The Bat ROS at the All-Star break) on a ROS-correct time base: TARGET_PA=275 / ROS_WEEKS=11 preserves the 25 PA/week framing, and players under 275 ROS PA are blended toward replacement. **This fixes a flaw in build_trade_tool.py, which kept TARGET_PA=625/NUM_WEEKS=25 with ROS data — with ASB projections (max ~290 PA) that would blend every player ~55% toward replacement and crush the star/scrub spread.** Pitchers are reused from trade_tool.html's PITCHERS array (ATC preseason — stale, labeled as such in the UI). Owned hitters missing from the projection file (Rooker, Laureano, Westburg) are synthesized at replacement level so rosters stay complete. Features: league power rankings with per-team "seller" checkboxes (persisted in localStorage, default = bottom 5), 14-category win-probability profile, "where to attack" weakest-category target suggestions, sortable/filterable trade-target list with Δ wins/wk (optimal-drop acquisition value), a Poach Board of seller teams' best assets, a Trade Builder that evaluates arbitrary multi-player trades for BOTH sides (re-optimizing both lineups) plus an auto "suggest 1-for-1 trades" sweep, and a full Teams view. All evaluation goes through pure functions (`evalHitting`/`evalPitching`/`evalTeam`) instead of mutate-and-restore. The lineup optimizer is NOT the old most-constrained-first greedy: it admits players best-value-first via augmenting paths (Kuhn's algorithm over the transversal matroid), which is provably the maximum-total-value legal lineup. The greedy version could strand a good bat on the bench behind a chain of movable players (e.g. McGonigle 3B/SS benched while a replacement-level placeholder held RF and Moniak LF/CF/RF sat at UTIL). Rebuild after new data: point the file constants at the new CSVs and run `python3 build_deadline_tool.py`.
