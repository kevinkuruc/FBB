...
19. Trade analysis tool: `trade_tool.html` (generated by `build_trade_tool.py` from `draft_tool.html` + `Post-Draft_Rosters.csv`). All 16 rosters are pre-loaded — every owned player and every FA stays in the pool as a potential trade target. `calculateMarginalValue` is rewritten to model an **acquisition**: for each candidate, we try every possible "drop" from Skrey's roster, re-run the position optimizer, and keep the swap that maximizes total expected wins. The "+MV" column is therefore (best post-trade wins − current wins). The position filter (C, 1B, ...) lets you sort all eligible targets by upside. Each player row carries an owner badge so you can see who you'd need to deal with. To refresh after roster moves: update `Post-Draft_Rosters.csv` and re-run `python3 build_trade_tool.py`.
