...
19. Trade analysis tool: `trade_tool.html` (generated by `build_trade_tool.py` from `draft_tool.html` + a Depth Charts ROS CSV + a rosters CSV). Hitter projections are recomputed inside the build script from `Depth_Charts_May_ROS.csv` (or whatever the latest ROS file is) using the same `TARGET_PA=625` supplement as `create_league_stats.py`, so weekly stats are per-PA × fixed PA/week and the math is unchanged when projection horizons shrink. All 16 rosters are pre-loaded from `rosters_May.csv` — every owned player and every FA stays in the pool as a potential trade target. `calculateMarginalValue` is rewritten to model an **acquisition**: for each candidate, we try every possible "drop" from Skrey's roster, re-run the position optimizer, and keep the swap that maximizes total expected wins. Multi-position bats like Arraez (1B/2B) preserve their flexibility automatically — adding a 1B-only target makes Arraez slide to 2B if that frees the best drop. The "+MV" column is therefore (best post-trade wins − current wins). The position filter (C, 1B, ...) lets you sort all eligible targets by upside. Each player row carries an owner badge so you can see who you'd need to deal with. To refresh after roster moves or new projections: drop the new files into `/home/user/FBB/`, point `MAY_ROS_FILE` / `ROSTERS_CSV` in `build_trade_tool.py` at them, and re-run `python3 build_trade_tool.py`.
